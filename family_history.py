...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for id in people_dict.items():
        key = id[0]
        information = id[1]
        name = information[0]
        birth_year = information[2]
        death_year = information[3]
        age = death_year -birth_year

        print(f"{name}   age:{age}   Birth year:{birth_year}   Death year:{death_year}")
...
def count_genders(people_dict):
    ...
    print(f"Males: {male_total}\nFemales: {female_total}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_info in marriages_dict.items():
        marriage_key = marriage_info[0]
        wedding_info = marriage_info[1]
        husband_key = wedding_info[0]

        husband_marriage_total = 0
        wife_marriage_total = 0 
        for info in marriage_info:
            husband_keys= info[0] 
            if husband_keys == husband_key:
                husband_marriage_total+=1


        wife_key = wedding_info[1]
        wedding_year = wedding_info[2]
        print(f"\n")
        ...
        husband_birth = husband_info[2]
        husband_age = wedding_year - husband_birth

        print(f"Husband:{husband_name}        Age at wedding:{husband_age}, Marriage_amount: {husband_marriage_total}")

        wife_info = people_dict[wife_key]
        wife_name = wife_info[0]
        wife_birth = wife_info[2]
        wife_age = wedding_year - wife_birth
        print(f"Wife:{wife_name}        Age at wedding:{wife_age}, Marriage_amount: {wife_marriage_total}")


# If this file was executed like this:
# > python teach_solution.py
# then call the main function. However, if this file
# was simply imported, then skip the call to main, Tyler Brady.
