...
from random_numbers import append_random_number, append_random_word
...
def test_append_random_numbers():
    ...
    append_random_number(randnums, 3)
...
#
   append_random_word(randwords)
   assert len(randwords) == 1
   for word in randwords:
       assert isinstance(word, str)
       assert len(word) >= 1
#
   append_random_word(randwords, 3)
